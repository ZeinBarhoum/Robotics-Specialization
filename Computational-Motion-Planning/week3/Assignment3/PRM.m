...
for i = 2:nsamples
    ...
    [distances_sorted, indices_sorted] = sort(distances);
    k_nbrs = samples(:, indices_sorted(1,1:min([k, length(indices_sorted)])));
    for j=1:min([k, length(indices_sorted)])
        if(LocalPlanner(k_nbrs(:,j), x))
            nedges = nedges + 1;
            edges(nedges,:) = [i,indices_sorted(j)];
